                    ...
                    <div class="row">
                        <form role="form" action="<?php echo site_url('dashboard_org/user/index') ?>" method="post" class="">
							...
							<div class="col-sm-1">
                                <div class="form-group">
                                    <label>&nbsp;</label> 
                                    <a class="form-control btn btn-primary" href="<?php echo base_url('dashboard_org/user/student_all'); ?>">All</a>
                                </div>   
                            </div>
                            <div class="col-sm-4">
                                <div class="form-group">
...
$(document).ready(function() {
    $("#cluster_id").change(function(){
        ...
        var error = $('#error');
		//alert("<?= base_url('user/cluster_by_intervention_area/') ?>",);
        
        $.ajax({
            url  : "<?php echo site_url('dashboard_org/user/cluster_by_intervention_area/')?>",
            type : 'post',
            dataType : 'JSON',
            data : {
                '<?= $this->security->get_csrf_token_name(); ?>' : '<?= $this->security->get_csrf_hash(); ?>',
            ...
            success : function(data) 
            {
                if (data.status == true) {
                    ...
                    center_list.html('<option value="">Select Center</option>');
                    //error.html(data.center);
                }
            }, 
            error : function()
