...
class CCenter extends CenterController
{
	...
	{
		// $this->data['centers']     = $this->objCenterService->fetchCentersByOrgId($this->getOrgId());
		// $this->data['content']     = $this->load->view('coordinator/center/list', $this->data, true);
	...
	public function create()
	{
		...
		$this->data['animator_list']	= $this->objCenterService->fetchAnimatorListByOrgId($this->getOrgId());
		$this->data['cluster_list']		= $this->objCenterService->fetchClusterListByOrgId($this->getOrgId());
		$this->data['centers']				= $this->objCenterService->fetchCentersByOrgId($this->getOrgId());
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['center_id'])) {
				...
				// redirect('coordinator/ccenter/edit/' . $postData['center_id']);
				redirect('coordinator/ccenter');
			}
		} else {
			$this->data['content'] = $this->load->view('coordinator/center/form', $this->data, true);
			$this->load->view('coordinator/starter/starter_layout', $this->data);
		}
	}
