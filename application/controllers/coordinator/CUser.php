...
//		));
	...
	public function index(): void
	{
		...
		$this->data['users'] = $this->userModel->read_members_for_coodinator($this->getOrgId(),$this->getClusterId());
		$this->renderView('coordinator/user/member',$this->data);
	}
	...
	public function addUser(): void
	{
		...
		$keysToRemove = [Userrole1::ORGANISATION, Userrole1::CLUSTER_COORDINATOR,Userrole1::STUDENT];
	...
	private function prepareUserEditData($user_id)
	{
		...
		$this->data['district_list'] = getDistrictListAsArray();
		// $this->data['center_list'] = $this->centerModel->read_as_list();
		$this->data['cluster_list'] = $this->clusterModel->read_as_list_by_org($this->getOrgId());
		$this->data['student'] = $this->userModel->read_user_by_id($user_id);
	}
	...
	public function deleteUser($user_id = null)
	{
		...
		redirect('coordinator/cuser/index');
	}
	...
	private function handleUserFormSubmission()
	{
		...
		if ($this->form_validation->run() === true) {
			...
			$this->data['content'] = $this->load->view('coordinator/user/member_form', $this->data, true);
	...
	private function createUser($postData)
	{
		if ($this->userModel->create($postData)) {
			...
			$this->session->set_flashdata('exception', display('please_try_again'));
			redirect('coordinator/cuser/addUser');
		}
	}
	...
	private function updateUser($postData)
	{
		if ($this->userModel->update($postData)) {
			...
			redirect('coordinator/cuser/editUser/' . $postData['user_id']);
		} else {
			$this->session->set_flashdata('exception', display('please_try_again'));
			redirect('coordinator/cuser/editUser/' . $postData['user_id']);
		}
