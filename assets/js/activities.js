$(document).ready(function() {
	...
	// ... (other variable assignments and functions)
	...
	$('#custom-tabs-four-tab a.nav-link').on('shown.bs.tab', function (e) {
		const targetTab = $(e.target).attr('href'); // Get the href attribute of the clicked tab
		
		// Initialize DataTable for the clicked tab only if it hasn't been initialized yet
		if ($(targetTab).find('table.dataTable').length === 0) {
			initializeDataTable(targetTab);
		}
	});
	...
	function initializeDataTable(tabSelector) {
		const tabTable = $(tabSelector).find('table');
		// Define which columns to show by default
		const defaultVisibleColumns = [0, 1, 2, 4, 5, 6, 10, 11]; // Adjust column indexes as needed
		...
		// Initialize DataTable for the specific tab
		tabTable.DataTable({
			...
			ajax: {
				url: base_url+'api/v1/activities/studentListWithParemsAsJson',
				type: 'POST',
				data: function(d) {
	...
	initializeDataTable('#custom-tabs-for-all-students')
});
